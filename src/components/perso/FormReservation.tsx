...
function FormReservation() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("http://www.lycee-ferry-versailles.fr:5173/src/php/Reservation/reservation.php", {
        method: "POST",
        body: formData,
      });
      if (!response.ok) {
  ...
  useEffect(() => {
    const fetchHoraires = async (date: string) => {
      try {
        const response = await fetch("http://www.lycee-ferry-versailles.fr:5173/src/php/ajax/getHoraires.php", {
          method: "POST",
          headers: {
  ...
  return (
    <div>
      <form className="form_container" method="post" onSubmit={handleSubmit}>
        ...
        {userData && <input type="hidden" name="userId" value={userData} />}

        <div className="form_control">
          <Input htmlFor="nom" labelValue="Nom" InputName="nom" InputId="nom" type="text" />
          <Input htmlFor="prenom" labelValue="Prénom" InputName="prenom" InputId="prenom" type="text" />
        </div>
        <Input htmlFor="classe" labelValue="Classe" InputName="classe" InputId="classe" type="text" />
        <SelectInputDate
          selectID="date"
          selectName="Date"
          options={optionsDate}
        ...
        />
        <SelectInputH selectID="horaire" selectName="Horaire" options={horaires} />
