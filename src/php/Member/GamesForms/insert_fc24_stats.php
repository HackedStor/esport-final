...
header("Access-Control-Allow-Origin: http://localhost:5173"); // Remplacez par le domaine de votre front-end
...
if ($data) {
    // Valider les données
    if (isset($data['user_id']) && isset($data['score']) && isset($data['scoreOtherTeam'])) {
        try {
            // Préparer une requête SQL pour insérer les données
            $stmt = $conn->prepare("INSERT INTO fc24_stats (user_id, score, other_team_score) VALUES (:user_id, :score, :otherTeamScore)");

            // Lier les paramètres et exécuter la requête
            $stmt->bindParam(':user_id', $data['user_id'], PDO::PARAM_INT);
            $stmt->bindParam(':score', $data['score'], PDO::PARAM_STR);
            $stmt->bindParam(':otherTeamScore', $data['scoreOtherTeam'], PDO::PARAM_STR);
