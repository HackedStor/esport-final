...
if ($data) {
    // Valider les données
    if (isset($data['user_id']) && isset($data['map_name']) && isset($data['score'])) {
        try {
            // Préparer une requête SQL pour insérer les données
            $stmt = $conn->prepare("INSERT INTO mk_stats (user_id, map_name, score) VALUES (:user_id, :map_name, :score)");

            // Lier les paramètres et exécuter la requête
            $stmt->bindParam(':user_id', $data['user_id'], PDO::PARAM_INT);
            $stmt->bindParam(':map_name', $data['map_name'], PDO::PARAM_STR);
